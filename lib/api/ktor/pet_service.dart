...
class PetService {
  ...
  Future<List<String>> getAdoptionRequestsForPet(int petId) async {
    final response = await http.get(
      Uri.parse('$ipAddress/adoptionrequests/$petId'),
      headers: {"Content-Type": "application/json"},
    );
  ...
  Future<bool> sendAdoptionRequest(int userId, int petId, String fullName) async {
  ...
  Future<bool> deleteAdoptionRequest(int requestId) async {
  final url = Uri.parse('$ipAddress/adoptionrequests/$requestId');
  try {
